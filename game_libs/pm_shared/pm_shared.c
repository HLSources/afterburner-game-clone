...
void PM_Duck( void )
{
	...
	if( ( pmove->cmd.buttons & IN_DUCK ) || ( pmove->bInDuck ) || ( pmove->flags & FL_DUCKING ) )
	{
		if( pmove->cmd.buttons & IN_DUCK )
		{
			...
			time = MAX_OF( 0.0, ( 1.0 - (float)pmove->flDuckTime / 1000.0 ) );
			...
			if( pmove->bInDuck )
			{
				...
				{
					pmove->usehull = 1;
					pmove->view_ofs[2] = VEC_DUCK_VIEW_HEIGHT;
					pmove->flags |= FL_DUCKING;
					pmove->bInDuck = false;
				...
				else
				{
					...
					pmove->view_ofs[2] = ( ( VEC_DUCK_VIEW_HEIGHT - fMore ) * duckFraction ) + ( VEC_VIEW * ( 1 - duckFraction ) );
				}
			}
		}
		else
...
void PM_CheckParamters( void )
{
	...
	if( maxspeed != 0.0 )
	{
		pmove->maxspeed = MIN_OF( maxspeed, pmove->maxspeed );
	}
